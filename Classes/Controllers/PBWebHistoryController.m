...
{
	currentOID = nil;
	[self callJavaScriptFunction:@"setRefColors" withArguments:@[ [PBGitRefLabelColors CSSColors] ] completionHandler:nil];
...
// WKWebView only accepts JSON-serializable arguments, unlike the legacy
// WebView, which let JavaScript call ObjC selectors directly on a bridged
// PBGitCommit/PBGitRef. Flatten both into plain dictionaries/strings before
// they cross the bridge; the keys line up with what history.js's Commit
// constructor and -showRefs read.
static NSArray<NSDictionary<NSString *, NSString *> *> *refsJSON(NSArray<PBGitRef *> *refs)
{
...
static NSDictionary *commitJSON(PBGitCommit *commit)
{
	NSMutableArray<NSString *> *parentSHAs = [NSMutableArray arrayWithCapacity:commit.parents.count];
	for (GTOID *parentOID in commit.parents) {
		[parentSHAs addObject:parentOID.SHA ?: @""];
	}

	return @{
		@"refs" : refsJSON(commit.refs),
...
- (void)changeContentToCommit:(PBGitCommit *)commit
{
	...
	if ([currentOID isEqual:commit.OID]) {
		NSArray *refreshed = @[ [PBWebHistoryController nameOfCurrentRefIn:repository], [repository refNamesHeldByOtherWorktrees], refsJSON(commit.refs) ];
		[self callJavaScriptFunction:@"reload" withArguments:refreshed completionHandler:nil];
		return;
	}

	NSArray *arguments = @[ commitJSON(commit), [PBWebHistoryController nameOfCurrentRefIn:repository], [repository refNamesHeldByOtherWorktrees] ];
