...
- (NSArray<NSMenuItem *> *)menuItemsForCommits:(NSArray<PBGitCommit *> *)commits
{
	...
	if (isSingleCommitSelection) {
		...
		NSString *checkoutCommitTitle = [NSString stringWithFormat:NSLocalizedString(@"Checkout TEST “%@”", @"Contextual Menu Item to check out the selected commit, %@ is the first part of its subject"), truncatedSubject];
